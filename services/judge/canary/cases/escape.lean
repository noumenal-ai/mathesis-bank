...
-- A sandbox-escape attempt: write outside the build directory, then reach the network.
-- Each must fail inside landrun; the harness checks the marker file never appears.
#eval (IO.FS.writeFile "/home/judge/ESCAPED" "written from Solution.lean" : IO Unit)
#eval (do
  let out ← IO.Process.output { cmd := "curl", args := #["-sS", "-m", "5", "https://example.com"] }
  IO.println s!"network exit {out.exitCode}, {out.stdout.length} bytes" : IO Unit)
