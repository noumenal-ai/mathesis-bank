...
-- Escapes a sandbox on a pre-7.1 kernel must stop: ask the user's systemd,
-- over its UNIX-socket bus, to start an unsandboxed process; and resolve a
-- name over UDP, which Landlock does not cover. Neither may succeed.
#eval (do
  let out ← IO.Process.output { cmd := "systemd-run", args := #["--user", "--wait", "--quiet", "touch", "/home/judge/ESCAPED"] }
  IO.println s!"systemd-run exit {out.exitCode}: {out.stderr}" : IO Unit)
#eval (do
  let out ← IO.Process.output { cmd := "getent", args := #["ahosts", "example.com"] }
  IO.println s!"dns exit {out.exitCode}: {out.stdout.length} bytes" : IO Unit)
